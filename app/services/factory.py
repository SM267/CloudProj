...
class SoftwareFactory:
    ...
    provider layer. This keeps the pipeline deterministic and testable with fakes.
    """
    ...
    async def plan(self, requirement: str, language: str, framework: str) -> SoftwarePlan:
        ...
        The current MVP keeps the baseline plan deterministic while calling the
        configured provider. Later stages can replace individual deterministic
        ...
        stages = [
            FactoryStage(
                ...
                summary="Requirement accepted",
            ),
            FactoryStage(
                name="architecture",
                status=StageStatus.SUCCEEDED,
                summary="Initial architecture produced",
            ),
            FactoryStage(
                name="planning",
                status=StageStatus.SUCCEEDED,
                summary="Implementation tasks created",
            ),
        ...
        await self.llm.generate(
            [
                LLMMessage(
                    role="system",
                    content=(
                        "You are the planning stage of a software factory. "
